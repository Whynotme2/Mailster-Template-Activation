...
if ( isset( $_GET['items'] ) ) {
	...
	if ( isset( $response['error'] ) ) {
		...
		$url = buildquery($_SESSION['returnto'], array(
			'mailster_error' => urlencode( $response['error_description'] ),
			'mailster_nonce' => $_SESSION['auth'],
		));


	} elseif ( isset( $response['refresh_token'] ) && isset( $response['access_token'] ) ) {
		...
		if ( isset( $response['error'] ) ) {
			...
			$url = buildquery($_SESSION['returnto'], array(
				'mailster_error' => urlencode( $error_description ),
				'mailster_slug' => $slug,
				'mailster_nonce' => $_SESSION['auth'],
			));

		} elseif ( isset( $response['download_url'] ) ) {
			...
			$url = buildquery($_SESSION['returnto'], array(
				'mailster_download_url' => urlencode( $download_url ),
				'mailster_slug' => $slug,
				'mailster_nonce' => $_SESSION['auth'],
			));
	...
	$url = buildquery($_SESSION['returnto'], array(
		'mailster_error' => urlencode( $_GET['error_description'] ),
		'mailster_nonce' => $_SESSION['auth'],
	));
